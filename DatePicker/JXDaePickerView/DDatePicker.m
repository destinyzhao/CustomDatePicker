...
#define RGB1(r, g, b) RGBA(r,g,b,1)
// 判断是否是iPhone X
...
#define KPickerViewHeight 240
#define kHeightOfButtonContentView 0.0
...
@interface DDatePicker ()<UIPickerViewDelegate,UIPickerViewDataSource,UIGestureRecognizerDelegate> {
    //日期存储数组
...
-(void)setupUI {
    ...
    // 初始化选择器视图控件
    _datePicker = [[UIPickerView alloc]initWithFrame:CGRectMake(0.0, 0.0, self.width, _contentView.height)];
...
-(void)addLabelWithName:(NSArray *)nameArr {
    ...
    if (!_dateLabelColor) {
        _dateLabelColor =  RGB1(247, 133, 51);
    }
